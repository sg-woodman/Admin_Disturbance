...
cad_poly_df %>% 
  ggplot(aes(x = mean_prop_dist, y = zone)) + 
  geom_boxplot() +
...
cad_poly_df %>% 
  group_by(zone) %>% 
  summarise(mean_dist = mean(mean_prop_dist, na.rm = T),
            sd_dist = sd(mean_prop_dist, na.rm = T),
            n_dist = n(),
            se_dist = sd_dist/sqrt(n_dist)) %>% 
  ggplot(aes(x = mean_dist, y = zone)) + 
  geom_col() + 
  geom_errorbar(aes(xmin = mean_dist - se_dist, xmax = mean_dist + se_dist)) +
  scale_x_continuous(expand = c(0,0),
                     limits = c(0, 0.1)) +
  xlab("Mean (±SE) proportion disturbed from 2001 to 2019") + 
  ylab("") + 
  theme_bw() + 
  theme(text = element_text(size=12))
...
summary(lm(sqrt(mean_prop_dist) ~ zone, data = cad_poly_df))
```

No, between the five adminstration zones in the study there appear to be no 
...
summary(lm(n_dist ~ zone, data = cad_poly_df))
modelbased::estimate_contrasts(lm(n_dist ~ zone, data = cad_poly_df))
```
